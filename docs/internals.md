...
### Server
The host that runs `redhat` in the server mode

### Client
The host behind the NAT that runs `redhat` in the client mode. It has some services that need to be forwarded.

### Visitor
Who visists a *service*, via the *server*

### Control Channel
A control channel is a TCP connection between the *server* and the *client* that only carries `redhat` control commands for one *service*.
...
When `redhat` starts in the client mode, it creates connections to `server.common.bind_addr` for each service. These connection acts as control channels.
