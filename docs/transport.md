...
By default, `redhat` forwards traffic as it is. Different options can be enabled to secure the traffic.
...
`redhat` provides optional `rustls` support. [Build Guide](build-guide.md) demostrated this.
...
`redhat` comes with a reasonable default configuration for noise protocol. You can a glimpse of the minimal [example](../examples/noise_nk) for how it will look like.

The default noise protocol that `redhat` uses, which is `Noise_NK_25519_ChaChaPoly_BLAKE2s`, providing the authentication of the server, just like TLS with properly configured certificates. So MITM is no more a problem.

To use it, a X25519 keypair is needed.

#### Generate a Keypair

1. Run `redhat --genkey`, which will generate a keypair using the default X25519 algorithm.
...
Then `redhat` will run under the protection of the Noise Protocol.

## Specifying the Pattern of Noise Protocol

The default configuration of Noise Protocol that comes with `redhat` satifies most use cases, which is described above. But there're other patterns that can be useful.
